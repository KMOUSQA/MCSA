...
/**
 * @author itshortly {@link http
 *         ://stackoverflow.com/questions/11187461/redirect-
 *         system-out-and-system-err-to-slf4j} v
 *
 */
...
public class SysStreamsLogger {
	...
	public static void bindSystemStreams() {
		// Enable autoflush
		System.setOut(new PrintStream(new LoggingOutputStream(sysOutLogger,
				false), true));
		System.setErr(new PrintStream(new LoggingOutputStream(sysErrLogger,
				true), true));
	}
	
	public static void bindSystemStreams(JTextArea textarea) {
		// Enable autoflush
		System.setOut(new PrintStream(new LoggingOutputStream(textarea,
				false), true));
		System.setErr(new PrintStream(new LoggingOutputStream(textarea,
				true), true));
	}
	...
	private static class LoggingOutputStream extends java.io.OutputStream {
		...
		public LoggingOutputStream(JTextArea textArea, boolean isError) {
			...
			if(textArea != null){
				{
			        ...
			        textArea.append(time + new String(theBytes));
			        // scrolls the text area to the end of data
			        textArea.setCaretPosition(textArea.getDocument().getLength());
				}
			}
